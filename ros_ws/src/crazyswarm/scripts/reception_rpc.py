...
def exe_color_detection(interface):
    ...
    if result is not None and len(result):
        ...
        horizon = (round(res[0]/240,3)-0.5)
        vertical = (round(res[1]/160,2)-0.5)
        temp = [horizon, vertical, res[2], rospy.get_rostime().to_sec()]
        msg.data = temp
        pub.publish(msg)
...
if __name__ == "__main__":
    ...
    interface = rpc.rpc_network_master(slave_ip="192.168.0.36", my_ip="192.168.0.54", port=0x1DBA)
    counter = 0
